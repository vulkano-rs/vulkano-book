...
> **Note**: The Vertex trait is used to describe the attributes of an individual vertex that can be 
> read by a vertex shader. It provides methods for specifying the format of the vertex's fields, 
> which can be done using field attributes like `format` and `name`. The trait can be derived using 
> the `Vertex` derive macro.
